...
import { IDisposableReference } from '../../yuagent/browser/aideAgentContentParts/aideAgentCollections.js';
...
export class PinnedContextWidget extends Disposable {
	...
	private refresh(): void {
		...
		if (pinnedContexts.length === 0) {
			this.message = localize(
				...
				"Pin files utk dicache dan dirujuk dengan LLM di sini."
			);
		} else {
	...
	render(parent: HTMLElement): void {
		...
		const buttonTitle = manageKbShortcut ? localize('managePinnedContextsKb', "Pinned Context ({0})", manageKbShortcut.getLabel()) : localize('managePinnedContextsNoKb', "Semat Konteks");
