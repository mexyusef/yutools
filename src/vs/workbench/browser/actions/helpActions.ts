...
class GetStartedWithAccessibilityFeatures extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('yutools.terminal_command_serve_llm', 'yutools.terminal_command_serve_llm'),
			category: Categories.Help,
			f1: true,
			menu: {
				id: MenuId.MenubarHelpMenu,
	...
	run(accessor: ServicesAccessor): void {
		...
		commandService.executeCommand('yutools.terminal_command_serve_llm');
	}
}
...
class BukaTerminalAtCwd extends Action2 {
	static readonly ID = 'workbench.action.bukaterminalAtCwd';
	constructor() {
		super({
			id: BukaTerminalAtCwd.ID,
			// title: localize2('getStartedWithAccessibilityFeatures', 'Get Started with Accessibility Features'),
			title: localize2('yutools.register_terminal_at_cwd', 'yutools.register_terminal_at_cwd'),
			category: Categories.Help,
			f1: true,
			menu: {
				id: MenuId.MenubarHelpMenu,
	...
	// perlu cari: run(accessor: ServicesAccessor)
	run(accessor: ServicesAccessor): void {
		const commandService = accessor.get(ICommandService);
		// commandService.executeCommand('workbench.action.openWalkthrough', 'SetupAccessibility');
		commandService.executeCommand('yutools.register_terminal_at_cwd');
	}
}
registerAction2(GetStartedWithAccessibilityFeatures);
registerAction2(BukaTerminalAtCwd);
