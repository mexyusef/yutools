 ...
 *--------------------------------------------------------------------------------------------*/
// jyw https://chat.mistral.ai/chat/a57125f3-5e65-44d2-ae8e-c46450d2d2cb
import { Disposable } from '../../../../../base/common/lifecycle.js';
import { ContentWidgetPositionPreference, ICodeEditor, IContentWidget } from '../../../editorBrowser.js';
...
export class ComboWebsitesWidget extends Disposable implements IContentWidget {
  private static readonly ID = 'editor.widget.comboWebsites';
  ...
  constructor(
    ...
    // @NativeEnvironmentService private readonly nativeEnvironmentService: NativeEnvironmentService,
  ) {
    super();
    this.theme = 'pink';
    this.backgroundText = 'Websites';
    // this.user_defined_id = 'comboWebsites';
    this.editor = editor;
    this.editorInfoProvider = this.instantiationService.createInstance(EditorInfoProvider);
    this.createStyles();
    this.domNode = document.createElement('div');
    this.domNode.className = 'combo-websites-widget futuristic-container';
    // jyw https://chat.mistral.ai/chat/a57125f3-5e65-44d2-ae8e-c46450d2d2cb
    // this.createInput();
    // this.editor.addContentWidget(this);
    // this.hide();
    this.createInput().then(() => {
      this.editor.addContentWidget(this);
      this.hide();
    });
  }
  ...
  private createStyles(): void {
    ...
    createCSSRule(`.combobox-websites-widget`, `
      position: relative;
      width: 400px;
      height: 50px;
    ...
    createCSSRule(`.combobox-websites-widget .input-container`, `
      display: flex;
      position: absolute;
      top: 0;
      left: 0;
    ...
    createCSSRule(`.combobox-websites-widget .futuristic-combobox`, `
      flex-grow: 1;
      background-color: ${this.getRGBA(themeColor, 0.1)};
      color: ${themeColor};
      font-size: 12px;
      border: none;
    ...
    createCSSRule(`.combobox-websites-widget .futuristic-combobox`, `
      ...
      scrollbar-color: ${this.getRGBA(themeColor, 0.5)} ${this.getRGBA(themeColor, 0.1)};
    `);

    createCSSRule(`.combobox-websites-widget .futuristic-combobox option`, `
      background-color: ${this.getRGBA(themeColor, 0.1)};
      color: ${themeColor};
    `);

    // Webkit browsers (Chrome, Safari) specific styles
    createCSSRule(`.combobox-websites-widget .futuristic-combobox::-webkit-scrollbar`, `
      width: 10px;
    `);

    createCSSRule(`.combobox-websites-widget .futuristic-combobox::-webkit-scrollbar-track`, `
      background: ${this.getRGBA(themeColor, 0.1)};
    `);

    createCSSRule(`.combobox-websites-widget .futuristic-combobox::-webkit-scrollbar-thumb`, `
      background-color: ${this.getRGBA(themeColor, 0.5)};
      border-radius: 20px;
      border: 3px solid ${this.getRGBA(themeColor, 0.1)};
    `);

    createCSSRule(`.combobox-websites-widget button`, `
      padding: 5px 10px;
      background-color: ${this.getRGBA(themeColor, 0.3)};
      color: ${themeColor};
      border: none;
    ...
    createCSSRule(`.combobox-websites-widget button:hover`, `
      background-color: ${this.getRGBA(themeColor, 0.5)};
    `);

    createCSSRule(`.combobox-websites-widget .background-text`, `
      position: absolute;
      top: 50%;
      left: 50%;
      ...
      font-size: 72px;
      font-weight: bold;
      color: ${this.getRGBA(themeColor, 0.1)};
      pointer-events: none;
      z-index: 1;
      user-select: none;
      white-space: nowrap;
    ...
    createCSSRule(`.combobox-websites-widget .futuristic-border`, `
      position: absolute;
      top: -2px;
      left: -2px;
      right: -2px;
      ...
      border: 2px solid ${this.getRGBA(themeColor, 0.5)};
      border-radius: 7px;
      pointer-events: none;
      animation: borderGlow 2s infinite alternate;
    `);

    createCSSRule(`.combobox-websites-widget .futuristic-glow`, `
      position: absolute;
      top: 0;
      left: 0;
      width: 100%;
      ...
      box-shadow: 0 0 20px ${this.getRGBA(themeColor, 0.3)};
      border-radius: 5px;
      pointer-events: none;
      opacity: 0.5;
      filter: blur(10px);
    ...
    createCSSRule(`@keyframes borderGlow`, `
      0% {
        ...
        box-shadow: 0 0 5px ${this.getRGBA(themeColor, 0.5)}, inset 0 0 5px ${this.getRGBA(themeColor, 0.5)};
      }
      100% {
        ...
        box-shadow: 0 0 20px ${this.getRGBA(themeColor, 0.8)}, inset 0 0 10px ${this.getRGBA(themeColor, 0.8)};
      }
    `);
  }

  // private async readOptionsFromFile(): Promise<string[]> {
  //   const filePath = path.join(os.homedir(), 'combobox-website-options.json');
  //   try {
  ...
  // }
  private async readOptionsFromFile(): Promise<string[]> {
    const homeDir = this.environmentService.userRoamingDataHome.fsPath;
    const filePath = joinPath(URI.file(homeDir), 'combobox-website-options.json');
    try {
      const fileContent = await this.fileService.readFile(filePath);
      const arrayBuffer = fileContent.value.buffer;
  ...
  // private async readOptionsFromFile(fileService: IFileService): Promise<string[]> {
  //   const homeDir = this.nativeEnvironmentService.homeDir;
  //   const filePath = joinPath(URI.file(homeDir), 'combobox-website-options.json');
  //   try {
  ...
  private async createInput(): Promise<void> {
    ...
    const options = await this.readOptionsFromFile();
  ...
  public setTheme(newTheme: string): void {
    ...
    this.createStyles();
    const themeColor = this.getThemeColor();
    // this.inputElement.style.backgroundColor = this.getRGBA(themeColor, 0.1);
    // this.inputElement.style.color = themeColor;
    // Update combobox styles
    this.comboboxElement.style.backgroundColor = this.getRGBA(themeColor, 0.1);
    this.comboboxElement.style.color = themeColor;

    // Update button styles
    this.buttonElement.style.backgroundColor = this.getRGBA(themeColor, 0.3);
    this.buttonElement.style.color = themeColor;

    const backgroundText = this.domNode.querySelector('.background-text') as HTMLElement;
    if (backgroundText) {
      backgroundText.style.color = this.getRGBA(themeColor, 0.1);
    }
  }
  ...
  private pulseEffect(): void {
    ...
    this.comboboxElement.style.boxShadow = `0 0 10px ${this.getRGBA(this.getThemeColor(), 0.8)}`;
    this.buttonElement.style.boxShadow = `0 0 10px ${this.getRGBA(this.getThemeColor(), 0.8)}`;
    setTimeout(() => {
  ...
  //   const activeTerminal = this.editorInfoProvider.getActiveTerminal();
  //   if (activeTerminal) {
  //     activeTerminal.sendText(text, true);
  //   } else {
  //     this.editorInfoProvider.error('No active terminal found.');
  //   }
  // }
