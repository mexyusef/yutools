...
// import { postToBackendAPI } from './fulled/utils/apiUtils';
import { KeyCode } from '../../../../../base/common/keyCodes.js';
import { IInstantiationService } from '../../../../../platform/instantiation/common/instantiation.js';
// import { executeVSCodeCommand } from './fulled/utils/commandUtils';
import {
  EditorInfoProvider,
...
export class Yuwidget3Widget extends Disposable implements IContentWidget {
  private static readonly ID = 'editor.widget.Yuwidget3Widget';
  ...
  constructor(
    ...
    registerWidgetTheme(Yuwidget3Widget.ID, 'teal');
    this.editor = editor;
    this.editorInfoProvider = this.instantiationService.createInstance(EditorInfoProvider);
    this.createStyles();
    this.domNode = document.createElement('div');
    this.domNode.className = 'combobox-terminal-widget futuristic-container';
    this.createInput();
  ...
  private createStyles(): void {
    const themeColor = getThemeColor(widgetThemeMap[Yuwidget3Widget.ID]);

    createCSSRule(`.combobox-terminal-widget`, `
      position: relative;
      width: 400px;
      height: 50px;
    ...
    createCSSRule(`.combobox-terminal-widget .input-container`, `
      display: flex;
      position: absolute;
      top: 0;
      left: 0;
    ...
    createCSSRule(`.combobox-terminal-widget .futuristic-combobox`, `
      flex-grow: 1;
    ...
    createCSSRule(`.combobox-terminal-widget .futuristic-combobox option`, `
      background-color: ${getRGBA(themeColor, 0.1)};
      color: ${themeColor};
    `);

    // Webkit browsers (Chrome, Safari) specific styles
    createCSSRule(`.combobox-terminal-widget .futuristic-combobox::-webkit-scrollbar`, `
      width: 10px;
    `);

    createCSSRule(`.combobox-terminal-widget .futuristic-combobox::-webkit-scrollbar-track`, `
      background: ${getRGBA(themeColor, 0.1)};
    `);

    createCSSRule(`.combobox-terminal-widget .futuristic-combobox::-webkit-scrollbar-thumb`, `
      background-color: ${getRGBA(themeColor, 0.5)};
      border-radius: 20px;
      border: 3px solid ${getRGBA(themeColor, 0.1)};
    `);

    createCSSRule(`.combobox-terminal-widget button`, `
      padding: 5px 10px;
    ...
    createCSSRule(`.combobox-terminal-widget button:hover`, `
      background-color: ${getRGBA(themeColor, 0.5)};
    `);

    createCSSRule(`.combobox-terminal-widget .background-text`, `
      position: absolute;
      top: 50%;
      left: 50%;
    ...
    createCSSRule(`.combobox-terminal-widget .futuristic-border`, `
      position: absolute;
      top: -2px;
      left: -2px;
      right: -2px;
    ...
    createCSSRule(`.combobox-terminal-widget .futuristic-glow`, `
      position: absolute;
      top: 0;
      left: 0;
      width: 100%;
  ...
  private handleResize(): void {
    ...
    // this.comboboxElement.style.height = '30px';
  ...
  private pulseEffect(): void {
    ...
    this.comboboxElement.style.boxShadow = `0 0 10px ${getRGBA(getThemeColor(widgetThemeMap[Yuwidget3Widget.ID]), 0.8)}`;
    this.buttonElement.style.boxShadow = `0 0 10px ${getRGBA(getThemeColor(widgetThemeMap[Yuwidget3Widget.ID]), 0.8)}`;
    setTimeout(() => {
