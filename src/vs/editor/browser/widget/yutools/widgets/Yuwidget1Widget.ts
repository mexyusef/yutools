...
export class Yuwidget1Widget extends Disposable implements IContentWidget {
	private static readonly ID = 'editor.widget.Yuwidget1Widget';
	...
	constructor(
		...
		registerWidgetTheme(Yuwidget1Widget.ID, 'lime');
		this.editor = editor;
	...
	private createStyles(): void {
		const themeColor = getThemeColor(widgetThemeMap[Yuwidget1Widget.ID]);
	...
	private pulseEffect(): void {
		...
		this.comboboxElement.style.boxShadow = `0 0 10px ${getRGBA(getThemeColor(widgetThemeMap[Yuwidget1Widget.ID]), 0.8)}`;
		this.buttonElement.style.boxShadow = `0 0 10px ${getRGBA(getThemeColor(widgetThemeMap[Yuwidget1Widget.ID]), 0.8)}`;
		setTimeout(() => {
