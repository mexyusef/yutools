	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: VsGentMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: VsGentMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: VsGentMessage[] = []
		let nextActionMessages: VsGentMessage[] = []
		...
		<div style={{ padding: "10px 6px 10px 15px", marginBottom: -10 }}>
			<div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
				...
				<span style={{ fontWeight: "bold" }}>
					<>VsGent wants to use the browser:</>
				</span>
			</div>
			<div
				style={{
