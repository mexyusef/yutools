...
import { extension_name, hidden_prompt_prefix, hidden_prompt_suffix } from '../constants';
...
export async function openFilesInEditor(filePaths: string[]) {
...
export async function createUntitledEditorWithText(content: string, language: 'plaintext'): Promise<void> {
	const document = await vscode.workspace.openTextDocument({ language, content });
	await vscode.window.showTextDocument(document);
}

export async function createSplitEditorWithText(content: string, language: 'plaintext'): Promise<void> {
	const document = await vscode.workspace.openTextDocument({ language, content });
	const activeEditor = vscode.window.activeTextEditor;
	const splitColumn = activeEditor ? activeEditor.viewColumn! + 1 : vscode.ViewColumn.Beside;
	await vscode.window.showTextDocument(document, { viewColumn: splitColumn, preview: false });
}