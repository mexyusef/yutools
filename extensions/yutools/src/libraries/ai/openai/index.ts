...
export async function handleOpenAIPrompt(prefix = '', suffix = '') {
  ...
  try {
    const responseText = await client.createCompletion({
      messages: [
        { role: 'system', content: openaiSettings.getConfig().systemPrompt as string },
...
export function register_openai_commands(context: vscode.ExtensionContext) {
  const sendPrompt = vscode.commands.registerCommand('yutools.llm.openai.sendPrompt', async () => {
    await handleOpenAIPrompt();
  });

  const sendPromptCode = vscode.commands.registerCommand('yutools.llm.openai.sendPromptCode', async () => {
    await handleOpenAIPrompt(getConfigValue('hiddenPromptPrefix', ''), getConfigValue('hiddenPromptSuffix', ''));
  });

  const sendPromptStream = vscode.commands.registerCommand('yutools.llm.openai.sendPromptStream', async () => {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
      vscode.window.showErrorMessage('No active editor found.');
      return;
  ...
  const sendPromptWithFilePrefix = vscode.commands.registerCommand('yutools.llm.openai.sendPromptWithFilePrefix', async () => {
    ...
    try {
      ...
      await handleOpenAIPrompt(prefix, '');
    } catch (error: any) {
