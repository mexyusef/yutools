...
import { glhfSettings, openaiSettings } from '@/libraries/ai/config';

export function register_diff_apply_glhf(context: vscode.ExtensionContext) {
  let disposable = vscode.commands.registerCommand('yutools.errors.diff.fixErrorsGLHF', async () => {
...
`;
      ...
      try {
        ...
        const response = await axios.post('https://glhf.chat/api/openai/v1/completions', {
          model: glhfSettings.getConfig().model, // Use the model from your settings
          prompt: prompt,
          max_tokens: 150 // Optional: Adjust based on your needs
        }, {
          headers: {
            'Authorization': `Bearer ${glhfSettings.getNextProvider().key}`, // Use the API key from your settings
