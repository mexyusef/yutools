...
import { hyperbolicSettings } from '@/libraries/ai/config';

export function register_diff_apply_hyperbolic(context: vscode.ExtensionContext) {
  let disposable = vscode.commands.registerCommand('yutools.errors.diff.fixErrorsHyperbolic', async () => {
...
`;
      ...
      try {
        const response = await axios.post('https://api.hyperbolic.xyz/v1/completions', {
          model: "deepseek-ai/DeepSeek-V3", // Use the specified model
          prompt: prompt,
          max_tokens: 150 // Optional: Adjust based on your needs
        }, {
          headers: {
            'Authorization': `Bearer ${hyperbolicSettings.getNextProvider().key}`, // Use the API key from your settings
