...
const PORT = 8200;
const ROOTDIR = 'C:\\ai\\fulled-templates\\react-app';
...
function startTerminalReact() {
	...
	const reactAppPath = ROOTDIR;
	serverTerminal = vscode.window.createTerminal('React App Server');
	serverTerminal.sendText(`cd "${reactAppPath}"`);
	...
	serverProcess.stdout?.on('data', (data) => {
		...
		if (data.toString().includes('Local:')) {
			...
			if (url && url[1]) {
				vscode.window.showInformationMessage(`React app is running at ${url[1]}`, 'Open in Browser').then(selection => {
...
export function registerLivePreviewsNextjsJavascript(context: vscode.ExtensionContext) {
	...
	// 🌺🕺🌟
	const commands = [
		{ command: `yutools.previews.startReactPreview`, rootDir: ROOTDIR, title: 'React🌍Preview', isNextJs: false },
	];
