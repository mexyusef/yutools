...
const PORT = 8201;
const ROOTDIR = 'C:\\ai\\fulled-templates\\react-app-ts';
...
function stopTerminalReact() {
	if (serverProcess) {
		serverProcess.kill();
		serverProcess = null;
		vscode.window.showInformationMessage('React app server stopped.');
	} else {
		vscode.window.showWarningMessage('No React app server is currently running.');
	}
}
...
function startTerminalReact() {
	...
	if (serverProcess) {
		vscode.window.showWarningMessage('React app server is already running.');
		return;
	}
	serverTerminal = vscode.window.createTerminal('React-TS App Server');
	...
	serverProcess.stdout?.on('data', (data) => {
		...
		// ambil alamat localhost: ... utk dibuka di browser
		if (data.toString().includes('Local:')) {
			const url = data.toString().match(/Local:\s+(http:\/\/[\w.:]+)/);
			if (url && url[1]) {
				vscode.window.showInformationMessage(`React-TS app is running at ${url[1]}`, 'Open in Browser').then(selection => {
	...
	vscode.window.showInformationMessage('React-TS app server started. Check the terminal for details.');
...
export function registerLivePreviewsReactjs(context: vscode.ExtensionContext) {
	...
	const commands = [
		{ command: `yutools.previews.startReactPreviewTS`, rootDir: ROOTDIR, title: 'React🌍Preview-TS', isNextJs: false },
	];
