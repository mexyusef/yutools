...
export class CodeApprovalLensProvider implements vscode.CodeLensProvider {
	...
	constructor() {
		...
		vscode.workspace.onDidChangeTextDocument((e) => {
			...
			if (this._diffsOfDocument[docUriStr]) {
				this._diffsOfDocument[docUriStr].splice(0);
			} else {
				// console.warn(`_diffsOfDocument[${docUriStr}] is undefined`);
				if (!this._diffsOfDocument[docUriStr]) {
					this._diffsOfDocument[docUriStr] = [];
					// console.debug(`Initialized _diffsOfDocument[${docUriStr}]`);
				}
			}
	...
	public async addNewApprovals(editor: vscode.TextEditor, suggestedEdits: SuggestedEdit[]) {
		...
		if (!this._diffsOfDocument[docUriStr]) { this._diffsOfDocument[docUriStr] = []; }
		if (!this._computedLensesOfDocument[docUriStr]) { this._computedLensesOfDocument[docUriStr] = []; }
		...
		for (let suggestedEdit of suggestedEdits) {
			...
			if (suggestedEdit.startLine < 0 || suggestedEdit.endLine < 0) {
				console.error(`Invalid line numbers: startLine=${suggestedEdit.startLine}, endLine=${suggestedEdit.endLine}`);
				vscode.window.showErrorMessage(`Invalid line numbers: startLine=${suggestedEdit.startLine}, endLine=${suggestedEdit.endLine}`);
				continue; // Skip this edit if invalid
			}

			// Handle case where startLine is greater than endLine
			if (suggestedEdit.startLine > suggestedEdit.endLine) {
				console.error(`startLine is greater than endLine: startLine=${suggestedEdit.startLine}, endLine=${suggestedEdit.endLine}`);
				continue;
			}
	...
	public async approveDiff({ diffid }: { diffid: number }) {
		...
		if (this._diffsOfDocument[docUriStr] && index >= 0) {
			this._diffsOfDocument[docUriStr].splice(index, 1);
		} else {
			console.error(`Error in approveDiff: Invalid index ${index} or undefined array for ${docUriStr}`);
		}
	...
	public async discardDiff({ diffid }: { diffid: number }) {
		...
		if (this._diffsOfDocument[docUriStr] && index >= 0) {
			...
			console.error(`Error in discardDiff: Invalid index ${index} or undefined array for ${docUriStr}`);
		}
